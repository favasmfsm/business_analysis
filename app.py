...
df1 = st.file_uploader("Upload Sales Register", type=["xlsx"])
df2 = st.file_uploader("Upload Purchase", type=["xlsx"])
...
df = pd.merge(df1, df2, on="Description", how="inner", suffixes=("_sales", "_purchase"))
...
filter_rate = st.number_input(
    "Enter the price for AUTO MOTIVE FILTTER", value=35, step=1
)
loose_oil_rate = st.number_input("Enter the price for LOOSE OIL", value=115, step=1)
if filter_rate:
...
selected_date = st.selectbox("Select a date", profit_df.index)
